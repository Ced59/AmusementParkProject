...
export class LeafletMapComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private initMap(): void {
    ...
    this.ngZone.runOutsideAngular((): void => {
      this.map?.on('moveend zoomend', () => this.handleMapViewportChange());
  ...
  private handleMapViewportChange(): void {
    if (!this.shouldRefreshMarkersAfterMapViewportChange()) {
      return;
    }

    this.scheduleMarkerRefresh(40);
  }

  private shouldRefreshMarkersAfterMapViewportChange(): boolean {
    return this.stabilizeDynamicMarkerViewport || this.shouldClusterMarkers();
  }
