...
describe('CountryDisplayService', () => {
  ...
  it('resolves localized country names when Intl.DisplayNames is available', () => {
    const result: string | null = service.resolveLocalizedCountryName(
      'FR',
      'en',
    );
  ...
  it('resolves country codes from direct ISO codes in a known collection', () => {
    ...
    expect(service.resolveCountryCodeFromLocalizedName(' be ', countries)).toBe(
      'BE',
    );
  });
  ...
  it('resolves country codes from accent-insensitive localized names', () => {
    ...
    expect(
      service.resolveCountryCodeFromLocalizedName(' allemagne ', countries),
    ).toBe('DE');
  });

  it('returns null when no country matches', () => {
    expect(
      service.resolveCountryCodeFromLocalizedName('Unknown', [
  ...
  it('checks country name matches accent-insensitively', () => {
    expect(service.countryNameMatches('Côte d’Ivoire', 'cote')).toBe(true);
    expect(service.countryNameMatches('Belgique', 'gique')).toBe(true);
    expect(service.countryNameMatches('Belgique', '')).toBe(false);
  });
});
