...
describe('ImageUploadSecurityService', () => {
  ...
  it('exposes an accept attribute matching accepted mime types', () => {
    expect(IMAGE_UPLOAD_ACCEPT_ATTRIBUTE).toBe(
      ACCEPTED_IMAGE_MIME_TYPES.join(','),
    );
  });

  it('rejects missing files', () => {
    expect(service.validateImageFile(null)).toEqual({
      isValid: false,
  ...
  it('rejects unsupported mime types case-insensitively', () => {
    ...
    expect(service.validateImageFile(file)).toEqual({
      isValid: false,
  ...
  it('rejects files bigger than the configured limit and accepts files exactly at the limit', () => {
    expect(
      service.validateImageFile(createFile('too-big.png', 'image/png', 11), 10)
        .errorKey,
    ).toBe('shared.imageUpload.invalidSize');
    expect(
      service.validateImageFile(createFile('exact.png', 'image/png', 10), 10)
        .isValid,
    ).toBe(true);
  ...
  it('filters only valid image files', () => {
    ...
    expect(
      service
        .filterValidImageFiles(files, 10)
