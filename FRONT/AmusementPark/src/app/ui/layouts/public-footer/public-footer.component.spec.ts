...
describe('PublicFooterComponent', () => {
  ...
  it('links the generated site version to the version history page', () => {
    const versionLink: HTMLAnchorElement | null = (
  ...
  it('renders the public brand wordmark with the .fun signature', () => {
    const brandLink: HTMLAnchorElement | null = (
      fixture.nativeElement as HTMLElement
    ).querySelector('.app-public-footer__logo');
    const logoImage: HTMLImageElement | null =
      brandLink?.querySelector('.app-public-footer__logo-mark') ?? null;
  ...
  it('renders the footer slogan as a synchronized data and fun phrase', () => {
    const slogan: HTMLElement | null = (
      fixture.nativeElement as HTMLElement
    ).querySelector('.app-public-footer__slogan');
    const dataLine: HTMLElement | null =
      slogan?.querySelector('.app-public-footer__slogan-base') ?? null;
    const funLine: HTMLElement | null =
      slogan?.querySelector('.app-public-footer__slogan-fun') ?? null;
  ...
  it('keeps the footer link columns after the full-width brand block', () => {
    const layout: HTMLElement | null = (
      fixture.nativeElement as HTMLElement
    ).querySelector('.app-public-footer__inner');
    const children: Element[] = Array.from(layout?.children ?? []);
    const brandIndex: number = children.findIndex((child: Element) =>
      child.classList.contains('app-public-footer__brand'),
    );
    const columnIndexes: number[] = children
      .map((child: Element, index: number) =>
        child.classList.contains('app-public-footer__column') ? index : -1,
      )
  ...
  it('marks the current footer language link for accessible active styling', () => {
    const activeLanguageLink: HTMLAnchorElement | null = (
      fixture.nativeElement as HTMLElement
