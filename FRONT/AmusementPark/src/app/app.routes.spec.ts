...
describe('App routes', () => {
  ...
  it('exposes the in-memory park-fit results without an authentication guard', () => {
    const route: Route | undefined = getPublicRoutes().find(
      (candidate: Route): boolean => candidate.path === 'park-fit/results'
    );
  ...
  it('exposes shared visit recaps as a dedicated public route before local passport drafts', () => {
    const publicRoutes: Route[] = getPublicRoutes();
    const sharedRouteIndex: number = publicRoutes.findIndex(
      (candidate: Route): boolean => candidate.path === 'passport/shared/visits/:shareId'
    );
