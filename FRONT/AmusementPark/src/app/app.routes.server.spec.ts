...
describe('Server routes', () => {
  ...
  it('server-renders shared year recaps', () => {
    const sharedYearRoute: ServerRoute | undefined = serverRoutes.find(
      (route: ServerRoute): boolean => route.path === ':lang/passport/shared/years/:shareId'
    );

    expect(sharedYearRoute?.renderMode).toBe(RenderMode.Server);
  });

  it('server-renders shared passport profiles', () => {
    const sharedProfileRoute: ServerRoute | undefined = serverRoutes.find(
      (route: ServerRoute): boolean => route.path === ':lang/passport/shared/profiles/:shareId'
    );
  ...
  it('keeps anonymous park-fit criteria out of server-rendered HTML', () => {
    ...
    const comparisonRoute: ServerRoute | undefined = serverRoutes.find(
      (candidate: ServerRoute): boolean => candidate.path === ':lang/park-fit/compare'
    );
    expect(comparisonRoute?.renderMode).toBe(RenderMode.Client);
