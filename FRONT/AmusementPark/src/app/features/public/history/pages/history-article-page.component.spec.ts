...
describe('HistoryArticlePageComponent', () => {
  ...
  beforeEach(async () => {
    ...
    seoService = {
      applyHistoryArticleSeo: vi
        .fn()
        .mockName('SeoService.applyHistoryArticleSeo'),
    } as unknown as MockedObject<SeoService>;
    routeParamMap = new BehaviorSubject<ParamMap>(
      convertToParamMap({ eventId: 'event-1' }),
    );
  ...
  it('uses the resolved article context for sharing and SEO', () => {
    ...
    expect(sharePanel.targetTitle).toBe(
      'Arrêt de Nitro - Le Nitro - Dennlys Parc',
    );
    expect(seoService.applyHistoryArticleSeo).toHaveBeenCalledWith(
      expect.objectContaining({
        title: 'Arrêt de Nitro',
        parkItem: expect.objectContaining({ name: 'Le Nitro' }),
        contextPark: expect.objectContaining({ name: 'Dennlys Parc' }),
      }),
      'fr',
      expect.any(String),
      expect.any(String),
