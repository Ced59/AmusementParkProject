...
describe('mapParkContentSummaryViewModel', () => {
  it('uses plural-aware label keys for totals, categories and attraction types', () => {
    const park: ParkDetailViewModel = {
      exploreLink: ['/fr/parc/1/demo/lieux'],
    ...
    const summary: ParkContentSummaryViewModel | null =
      mapParkContentSummaryViewModel(park, explorer);
    ...
    expect(summary?.entries).toContainEqual(
      expect.objectContaining({
        labelKey: `${publicCountsPrefix}place`,
        count: 3,
      }),
    );
    expect(summary?.entries).toContainEqual(
      expect.objectContaining({
        labelKey: `${homeCountsPrefix}show`,
        count: 1,
      }),
    );
    expect(summary?.entries).toContainEqual(
      expect.objectContaining({
        labelKey: `${homeCountsPrefix}shop`,
        count: 2,
      }),
    );
    expect(summary?.entries).toContainEqual(
      expect.objectContaining({
        labelKey: `${publicCountsPrefix}rollerCoaster`,
        count: 1,
      }),
    );
  });
