...
describe('park-reference-manufacturer-upsert.mapper', () => {
  it('builds a park-independent JSON upsert draft for the current manufacturer', () => {
    ...
    const document = JSON.parse(
      buildManufacturerParkGraphUpsertJson(manufacturer),
    );
    ...
    expect(draftManufacturer).toEqual(
    ...
    expect(draftManufacturer.contactDetails).toEqual(
      expect.objectContaining({
        websiteUrl: 'https://mack-rides.com',
        countryCode: 'DE',
        latitude: 48.091,
  ...
  it('builds a stable import file name from the manufacturer identity', () => {
    ...
    ).toBe('a-r-m-manufacturer-upsert.json');
  });
});
