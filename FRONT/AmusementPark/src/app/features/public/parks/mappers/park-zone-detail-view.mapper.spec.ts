...
describe('mapParkZoneToDetailViewModel', () => {
  it('maps localized zone details, counts, coordinates and query params', () => {
    ...
    const result = mapParkZoneToDetailViewModel(
      zone,
      bucket,
      ['/', 'fr'],
      'fr',
  ...
  it('falls back to raw zone name, id and default visibility when localized values are missing', () => {
    ...
    expect(
      mapParkZoneToDetailViewModel({ id: 'z2', parkId: 'p1' }, null, null, 'en')
        .name,
    ).toBe('z2');
    expect(
      mapParkZoneToDetailViewModel({ parkId: 'p1' }, null, null, 'en').name,
    ).toBe('');
    expect(
      mapParkZoneToDetailViewModel({ parkId: 'p1' }, null, null, 'en')
        .isVisible,
    ).toBe(true);
  });
});
