...
import { FakeParksPort } from './test-helpers/park-detail-state.facade/fake-parks-port';
...
describe('ParkDetailStateFacade', () => {
  ...
  it('loads the optimized park detail summary and nearby parks', () => {
    ...
    expect(context.facade.park()?.videosLink).toEqual([
      '/',
      'fr',
      'park',
      'park-1',
    ...
    expect(
      context.facade
        .summary()
        ?.entries.find((entry) => entry.labelKey === 'publicCounts.place')
        ?.count,
    ).toBe(3);
    ...
    expect(context.videosPort.itemVideoCalls).toEqual([
      { parkId: 'park-1', query: { page: 1, size: 1 } },
    ]);
    expect(context.parksPort.nearestCalls).toEqual([
      { sourceParkId: 'park-1', limit: 4, maxDistanceKilometers: null },
    ]);
    ...
    expect(
      context.facade.nearbyParks()[0].shortDescription?.length,
  ...
  it('exposes the videos link when only park items have videos', () => {
    const context = configureFacade();
    context.videosPort.videosResponse$ = of(createVideosPage(0));
    context.videosPort.itemVideosResponse$ = of(
    ...
    expect(
      capturedOptions.every(
        (options: AnonymousHttpOptions | undefined) =>
          options?.context.get(SKIP_AUTHORIZATION_HEADER) === true,
      ),
    ).toBe(true);
  });
  ...
  it('loads a bounded future opening window when the preview has no upcoming opening', () => {
    ...
    expect(
      context.facade
        .openingHours()
        ?.days.map((day: ParkOpeningHoursDay) => day.localDate),
  ...
  it('exposes the images link when only park items have photos', () => {
    ...
    context.imagesPort.itemImagesResponse$ = of(
