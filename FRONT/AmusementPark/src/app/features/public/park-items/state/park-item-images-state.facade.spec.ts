...
describe('ParkItemImagesStateFacade', () => {
  ...
  it('loads the item, parent park and first image page through public ports', () => {
    ...
    expect(context.imagesPort.pageCalls).toEqual([
      { ownerType: ImageOwnerType.ATTRACTION, ownerId: 'item-1', category: ImageCategory.ATTRACTION, page: 1, size: 24 }
    ]);
  });
  ...
  it('appends the next image page', () => {
    ...
    expect(context.imagesPort.pageCalls).toEqual([
      { ownerType: ImageOwnerType.ATTRACTION, ownerId: 'item-1', category: ImageCategory.ATTRACTION, page: 1, size: 24 },
      { ownerType: ImageOwnerType.ATTRACTION, ownerId: 'item-1', category: ImageCategory.ATTRACTION, page: 2, size: 24 }
    ]);
  });
