...
describe('HomeViewComponent', () => {
  ...
  it('limits mobile search suggestions to five current results', () => {
    const fixture: ComponentFixture<HomeViewComponent> = createComponent(
      createSearchResults(6),
    );

    fixture.detectChanges();

    const suggestionElements: NodeListOf<Element> =
      fixture.nativeElement.querySelectorAll('.home-search-suggestion');
  ...
  it('emits a suggestion title when a result has no direct public link', () => {
    ...
    const suggestionButton: HTMLButtonElement = fixture.debugElement.query(
      By.css('button.home-search-suggestion'),
    ).nativeElement;
  ...
  it('links standalone attraction suggestions to their public detail page', () => {
    const fixture: ComponentFixture<HomeViewComponent> = createComponent([
      createSearchResult(
        'standaloneAttraction_standalone-1',
    ...
    const suggestionLink: HTMLAnchorElement = fixture.debugElement.query(
      By.css('a.home-search-suggestion'),
    ).nativeElement;

    expect(suggestionLink.getAttribute('href')).toContain(
      '/en/attraction/standalone-1/bardonecchia-alpine-coaster',
    );
  });
  ...
  it('emits the park autocomplete title when the autocomplete is clicked', () => {
    ...
    fixture.componentInstance.autocompleteSelected.subscribe(
      (title: string) => {
        selectedTitle = title;
      },
    );
    ...
    const autocompleteButton: HTMLButtonElement = fixture.debugElement.query(
      By.css('button.home-search-autocomplete'),
    ).nativeElement;
  ...
  it('shows up to three park autocomplete names from current results', () => {
    const fixture: ComponentFixture<HomeViewComponent> = createComponent(
      [
        createSearchResult('park_1', 'Park', 'Boudewijn Seapark'),
        createSearchResult('parkItem_1', 'Attraction', 'Boud Hotel', {
          parentParkName: 'Boud Parent Park',
        }),
    ...
    const autocompleteButtons: NodeListOf<HTMLButtonElement> =
      fixture.nativeElement.querySelectorAll('button.home-search-autocomplete');
    const autocompleteTexts: string[] = Array.from(autocompleteButtons).map(
      (button: HTMLButtonElement) => button.textContent ?? '',
    );

    expect(autocompleteButtons.length).toBe(3);
    expect(
      autocompleteTexts.some((text: string) =>
        text.includes('Boudewijn Seapark'),
      ),
    ).toBe(true);
    expect(
      autocompleteTexts.some((text: string) =>
        text.includes('Boud Parent Park'),
      ),
    ).toBe(true);
    expect(
      autocompleteTexts.some((text: string) =>
        text.includes('Boud Adventure World'),
      ),
    ).toBe(true);
    expect(
      autocompleteTexts.some((text: string) => text.includes('Boud Hotel')),
    ).toBe(false);
    expect(
      autocompleteTexts.some((text: string) => text.includes('Boud Park Four')),
    ).toBe(false);
  });
  ...
  it('uses parent park names as autocomplete candidates for broad search results', () => {
    const fixture: ComponentFixture<HomeViewComponent> = createComponent(
      [
        createSearchResult('parkItem_1', 'Attraction', 'Taron', {
          parentParkName: 'Phantasialand',
        }),
      ],
      'Phan',
    );
...
): ComponentFixture<HomeViewComponent> {
  const fixture: ComponentFixture<HomeViewComponent> =
    TestBed.createComponent(HomeViewComponent);
  const component: HomeViewComponent = fixture.componentInstance;
  const readyState: Signal<ScreenState<unknown, string>> = signal<
    ScreenState<unknown, string>
  >({ kind: 'ready' }).asReadonly();
