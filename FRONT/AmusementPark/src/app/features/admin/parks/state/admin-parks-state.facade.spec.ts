...
import { FakeParksPort } from './test-helpers/admin-parks-state.facade/fake-parks-port';
...
describe('AdminParksStateFacade', () => {
  ...
  it('switches to search when a query is set', () => {
    ...
    expect(facade.parks().map((park: Park) => park.id)).toEqual([
      'searched-park',
    ]);
  });
  ...
  it('applies filters and resets to the first page', () => {
    ...
    expect(facade.visibilityFilter()).toBe(true);
    expect(facade.countryCodeFilter()).toBe('DE');
    expect(facade.validCoordinatesFilter()).toBe(true);
  ...
  it('passes admin sort options to park list requests', () => {
    ...
    expect(port.pageCalls[0].sort).toEqual({
      sortBy: 'parkItemsVisibleCount',
      sortDirection: 'desc',
    });
  });
  ...
  it('delegates bulk administration updates to the port', () => {
    ...
    facade
      .updateBulkAdministration(request)
      .subscribe((result: BulkAdministrationUpdateResult) => {
  ...
  it('makes filtered valid-coordinate parks visible through criteria bulk update', () => {
    ...
    facade
      .makeFilteredValidCoordinateParksVisible()
      .subscribe((result: BulkAdministrationUpdateResult) => {
        expect(result.updatedCount).toBe(1);
      });
