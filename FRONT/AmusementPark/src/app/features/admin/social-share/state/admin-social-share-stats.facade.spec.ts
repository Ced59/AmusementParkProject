...
describe('AdminSocialShareStatsFacade', () => {
  ...
  let port: MockedObject<AdminSocialShareStatsDataPort>;
  ...
  beforeEach(() => {
    port = {
      getStats: vi.fn().mockName('AdminSocialShareStatsDataPort.getStats'),
    } as unknown as MockedObject<AdminSocialShareStatsDataPort>;
  ...
  it('keeps previous stats available when a reload fails', () => {
    ...
    port.getStats.mockReturnValue(
      throwError(
        () => new Error('network'),
      ) as Observable<SocialShareStatsResult>,
    );
    facade.load();
