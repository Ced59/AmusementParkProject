...
describe('AdminPublicViewSimulationInterceptor', () => {
  function captureRequest(
    interceptor: AdminPublicViewSimulationInterceptor,
  ...
  it('keeps anonymous public requests unchanged by default', async () => {
    ...
    const capturedRequest: HttpRequest<unknown> = await captureRequest(
      interceptor,
      request,
    );

    expect(
      capturedRequest.headers.has('X-AmusementPark-Public-View-Mode'),
    ).toBe(false);
  ...
  it('adds the simulated role header and allows auth for user visitor mode', async () => {
    ...
    const capturedRequest: HttpRequest<unknown> = await captureRequest(
      interceptor,
      createAnonymousGetRequest(),
    );

    expect(
      capturedRequest.headers.get('X-AmusementPark-Public-View-Mode'),
    ).toBe('userVisitor');
