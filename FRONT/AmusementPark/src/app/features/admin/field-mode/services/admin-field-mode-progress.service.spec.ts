...
describe('AdminFieldModeProgressService', () => {
  ...
  it('loads processed ids from the field-mode endpoint and mirrors them locally', () => {
    ...
    const request = httpTestingController.expectOne(
      `${environment.apiBaseUrl}admin/field-mode/parks/park%201/processed-items`,
    );
    ...
    expect(
      JSON.parse(
        localStorage.getItem('admin.fieldMode.processedItemIds.park 1') ?? '[]',
      ),
    ).toEqual(['item-1']);
  });
  ...
  it('updates processed state through the field-mode endpoint and stores a local fallback', () => {
    service
      .setProcessed('park 1', 'item 1', true)
      .subscribe((isProcessed: boolean) => {
    ...
    const request = httpTestingController.expectOne(
      `${environment.apiBaseUrl}admin/field-mode/parks/park%201/items/item%201/processed`,
    );
  ...
  it('falls back to local processed ids when the field-mode endpoint fails', () => {
    localStorage.setItem(
      'admin.fieldMode.processedItemIds.park-1',
    ...
    const request = httpTestingController.expectOne(
      `${environment.apiBaseUrl}admin/field-mode/parks/park-1/processed-items`,
    );
