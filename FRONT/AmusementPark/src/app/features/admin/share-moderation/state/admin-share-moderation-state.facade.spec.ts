...
describe('AdminShareModerationStateFacade', (): void => {
  ...
  it.each([
    ['Dismiss', 'Pending'],
    ['Suspend', 'Pending'],
    ['Restore', 'PublicationSuspended'],
  ] as const)(
    'polls the active filter until a queued %s decision has converged',
    async (
      decision: ShareModerationDecision,
      statusBeforeDecision: ShareModerationReport['status'],
    ): Promise<void> => {
      vi.useFakeTimers();
      const report: ShareModerationReport = {
        reportId: 'report-1', targetType: 'VisitRecap', reason: 'PersonalData',
        details: 'A phone number is visible.', status: statusBeforeDecision,
      ...
      facade.load(query);
      facade.review('report-1', decision, ' Confirmed ');

      expect(facade.reports()).toEqual([report]);
      expect(facade.reviewingReportId()).toBe('report-1');
      expect(reviewRequest).toEqual({ decision, note: 'Confirmed' });
