...
interface ParkMapSeoCopy {
  ...
  officialDescription: (parkName: string, locationLabel: string) => string;
}
...
const PARK_MAP_SEO_COPY: Record<string, ParkMapSeoCopy> = {
  en: {
    ...
    officialTitlePrefix: 'Official maps of',
    parkFallback: 'this park',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Browse the official maps of ${parkName}${locationLabel ? ` in ${locationLabel}` : ''}, organized by year.`
  },
  fr: {
    titlePrefix: 'Carte de',
    interactiveTitlePrefix: 'Carte interactive de',
    officialTitlePrefix: 'Plans officiels de',
    parkFallback: 'ce parc',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Consulte les plans officiels de ${parkName}${locationLabel ? ` à ${locationLabel}` : ''}, classés par année.`
  },
  es: {
    titlePrefix: 'Mapa de',
    interactiveTitlePrefix: 'Mapa interactivo de',
    officialTitlePrefix: 'Mapas oficiales de',
    parkFallback: 'este parque',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Consulta los mapas oficiales de ${parkName}${locationLabel ? ` en ${locationLabel}` : ''}, organizados por año.`
  },
  de: {
    titlePrefix: 'Karte von',
    interactiveTitlePrefix: 'Interaktive Karte von',
    officialTitlePrefix: 'Offizielle Parkpläne von',
    parkFallback: 'diesem Park',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Sieh dir die offiziellen Parkpläne von ${parkName}${locationLabel ? ` in ${locationLabel}` : ''} nach Jahr an.`
  },
  it: {
    titlePrefix: 'Mappa di',
    interactiveTitlePrefix: 'Mappa interattiva di',
    officialTitlePrefix: 'Mappe ufficiali di',
    parkFallback: 'questo parco',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Consulta le mappe ufficiali di ${parkName}${locationLabel ? ` a ${locationLabel}` : ''}, organizzate per anno.`
  },
  nl: {
    titlePrefix: 'Kaart van',
    interactiveTitlePrefix: 'Interactieve kaart van',
    officialTitlePrefix: 'Officiële parkplattegronden van',
    parkFallback: 'dit park',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Bekijk de officiële parkplattegronden van ${parkName}${locationLabel ? ` in ${locationLabel}` : ''}, gerangschikt per jaar.`
  },
  pl: {
    titlePrefix: 'Mapa',
    interactiveTitlePrefix: 'Interaktywna mapa',
    officialTitlePrefix: 'Oficjalne mapy parku',
    parkFallback: 'tego parku',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Przeglądaj oficjalne mapy parku ${parkName}${locationLabel ? ` w ${locationLabel}` : ''}, uporządkowane według roku.`
  },
  pt: {
    titlePrefix: 'Mapa de',
    interactiveTitlePrefix: 'Mapa interativo de',
    officialTitlePrefix: 'Mapas oficiais de',
    parkFallback: 'este parque',
    ...
    officialDescription: (parkName: string, locationLabel: string): string => `Consulta os mapas oficiais de ${parkName}${locationLabel ? ` em ${locationLabel}` : ''}, organizados por ano.`
  }
};
...
const STATIC_SEO_COPY: Record<string, Record<string, StaticSeoCopy>> = {
  en: {
    ...
    ratingMethodology: {
      title: 'How visitor rankings are calculated — Amusement Parks',
      description: 'Understand the public rating scale, Bayesian score, evidence thresholds, park weights, ties and methodology history behind visitor rankings.',
    },
    technical: {
  ...
  fr: {
    ...
    ratingMethodology: {
      title: 'Comment les classements sont calculés — Amusement Parks',
      description: 'Comprends l’échelle de notes, le score bayésien, les seuils de preuve, les pondérations des parcs, les égalités et l’historique de la méthodologie.',
    },
    technical: {
  ...
  es: {
    ...
    ratingMethodology: { title: 'Cómo se calculan las clasificaciones — Amusement Parks', description: 'Conoce la escala, la puntuación bayesiana, los umbrales de prueba, las ponderaciones, los empates y el historial de la metodología.' },
  ...
  de: {
    ...
    ratingMethodology: { title: 'So werden Ranglisten berechnet — Amusement Parks', description: 'Verstehe Skala, bayesschen Wert, Evidenzschwellen, Parkgewichtung, Gleichstände und den Verlauf der Ranglisten-Methodik.' },
  ...
  it: {
    ...
    ratingMethodology: { title: 'Come vengono calcolate le classifiche — Amusement Parks', description: 'Comprendi scala, punteggio bayesiano, soglie di prova, pesi dei parchi, parità e cronologia della metodologia.' },
  ...
  pl: {
    ...
    ratingMethodology: { title: 'Jak obliczamy rankingi — Amusement Parks', description: 'Poznaj skalę ocen, wynik bayesowski, progi dowodów, wagi parków, remisy i historię metodologii.' },
  ...
  nl: {
    ...
    ratingMethodology: { title: 'Hoe ranglijsten worden berekend — Amusement Parks', description: 'Begrijp de schaal, Bayesiaanse score, bewijsdrempels, parkweging, gelijke standen en geschiedenis van de methodologie.' },
  ...
  pt: {
    ...
    ratingMethodology: { title: 'Como são calculadas as classificações — Amusement Parks', description: 'Compreende a escala, a pontuação bayesiana, os limiares de prova, os pesos dos parques, os empates e o histórico da metodologia.' },
...
export class SeoService {
  ...
  applyParkFitResultsSeo(
    ...
    this.apply({
      ...
      jsonLd: [this.buildBreadcrumbJsonLd([
        {
          name: normalizeSeoText(homeLabel, 'Home'),
          url: this.canonicalUrlService.buildAbsoluteUrl(`/${language}/home`)
        },
        {
          name: normalizeSeoText(parkFitLabel, 'Park Fit'),
          url: this.canonicalUrlService.buildAbsoluteUrl(`/${language}/park-fit`)
        },
        {
          name: normalizeSeoText(resultsLabel, title),
          url: canonicalUrl
        }
      ])]
    });
  ...
  applyCommentsSeo(
    ...
    const isIndexable: boolean = !this.hasQueryString(url) && thread.comments.length > 0;
  ...
  applyParkMapSeo(
    ...
    const locationLabel: string = this.buildLocalizedLocationLabel(park, normalizedLanguage);
    const titleSuffix: string = locationLabel ? ` — ${locationLabel}` : '';
    const hasInteractiveMarkers: boolean = markerCount > 0;
    const description: string = hasOfficialMaps
      ? hasInteractiveMarkers
        ? copy.description(parkName, locationLabel)
        : copy.officialDescription(parkName, locationLabel)
      : copy.interactiveDescription(parkName, locationLabel);
    const titlePrefix: string = hasOfficialMaps
      ? hasInteractiveMarkers
        ? copy.titlePrefix
        : copy.officialTitlePrefix
      : copy.interactiveTitlePrefix;
  ...
  applyParkWeatherSeo(
    ...
    const isIndexable: boolean = !this.hasQueryString(url) && totalDays > 0;
    ...
    this.apply({
      ...
      imageAlt: normalizedParkName,
      jsonLd: isIndexable
        ? [this.buildParkSubpageBreadcrumbJsonLd({ name: normalizedParkName } as Park, seoUrl, this.resolveParkWeatherBreadcrumbLabel(normalizedLanguage, normalizedParkName))]
        : []
    });
  }
  ...
  applyParkOpeningHoursSeo(
    ...
    this.apply({
      ...
      imageAlt: normalizedParkName,
      jsonLd: isIndexable
        ? [this.buildParkSubpageBreadcrumbJsonLd({ name: normalizedParkName } as Park, seoUrl, this.resolveParkOpeningHoursBreadcrumbLabel(normalizedLanguage, normalizedParkName))]
        : []
    });
  }
  ...
  applyParkPricingSeo(
    ...
    const title: string = `${copy.title(normalizedParkName)} - ${SITE_NAME}`;
    const description: string = copy.description(normalizedParkName);
    const isIndexable: boolean = !this.hasQueryString(url) && offerCount > 0;
  ...
  applyParkZoneSeo(
    ...
    this.apply({
      ...
      jsonLd: isIndexable
        ? [this.buildParkSubpageBreadcrumbJsonLd({ name: normalizedParkName } as Park, seoUrl, normalizedZoneName)]
        : []
    });
  }
