...
describe('SeoService', () => {
  ...
  it('uses the park primary photo as the Open Graph image', () => {
    ...
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/park-photo%201/social-preview-v2',
    );
  ...
  it('uses natural French prepositions for a park city and country', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Guide de Denain Évasion à Denain, en France — Amusement Parks',
    );
  ...
  it('avoids saying a park is directly à a country in French', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Guide de Demo Park en France — Amusement Parks',
    );
    expect(readMetaContent('meta[property="og:title"]')).not.toContain(
      ' à France',
    );
  });
  ...
  it('uses au for French park metadata in a masculine country', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Guide de Demo Park à Montréal, au Canada — Amusement Parks',
    );
    expect(readMetaContent('meta[property="og:description"]')).toContain(
      'Demo Park à Montréal, au Canada',
    );
  });
  ...
  it('uses aux for French park metadata in a plural country', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Guide de Demo Park à Orlando, aux États-Unis — Amusement Parks',
    );
    expect(readMetaContent('meta[property="og:description"]')).toContain(
      'Demo Park à Orlando, aux États-Unis',
    );
  });
  ...
  it('uses neutral location context in other languages to avoid country grammar errors', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Parkprofil Demo Park — Zürich, Schweiz — Amusement Parks',
    );
    expect(readMetaContent('meta[property="og:description"]')).toContain(
      'Demo Park — Zürich, Schweiz',
    );
    expect(readMetaContent('meta[property="og:description"]')).not.toContain(
      'in Schweiz',
    );
  ...
  it('uses the park item hero photo as the Open Graph image', () => {
    ...
    service.applyParkItemDetailSeo(
      detail,
      'fr',
      '/fr/park/park-1/demo-park/item/item-1/demo-item',
    );

    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/item-photo-1/social-preview-v2',
    );
    expect(readMetaContent('meta[name="twitter:image"]')).toBe(
      'https://localhost:44391/images/binary/item-photo-1/social-preview-v2',
    );
  ...
  it('removes stale social image dimensions when a responsive image replaces the fallback', () => {
    service.applyParkDetailSeo(
      buildParkDetail({ primaryPhoto: null }),
      'fr',
      '/fr/park/park-1/demo-park',
    );
  ...
  it('uses the park gallery photo as the Open Graph image without falling back to the park logo', () => {
    ...
    service.applyParkImagesSeo(
      park,
      'fr',
      '/fr/park/park-1/demo-park/photos',
      4,
      'gallery image 1',
    );

    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/gallery%20image%201/social-preview-v2',
    );
    expect(readMetaContent('meta[name="twitter:image"]')).toBe(
      'https://localhost:44391/images/binary/gallery%20image%201/social-preview-v2',
    );
  ...
  it('uses the park main photo when a park video has no thumbnail', () => {
    ...
    service.applyParkVideoSeo(
      ...
      buildPark(),
      'fr',
      '/fr/park/park-1/demo-park/videos/video-1/demo-video',
      'park-main-1',
    );

    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/park-main-1/social-preview-v2',
    );
    expect(readMetaContent('meta[name="twitter:image"]')).toBe(
      'https://localhost:44391/images/binary/park-main-1/social-preview-v2',
    );
  ...
  it('uses the park item main photo when a park item video has no thumbnail', () => {
    ...
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/item-main-1/social-preview-v2',
    );
    expect(readMetaContent('meta[name="twitter:image"]')).toBe(
      'https://localhost:44391/images/binary/item-main-1/social-preview-v2',
    );
  ...
  it('uses the first video thumbnail before the park main photo on video lists', () => {
    ...
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/video-thumb-1/social-preview-v2',
    );
    expect(readMetaContent('meta[name="twitter:image"]')).toBe(
      'https://localhost:44391/images/binary/video-thumb-1/social-preview-v2',
    );
  ...
  it('uses localized French fallbacks for park item video social metadata', () => {
    ...
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Regarde Demo video pour River Quest à Phantasialand.',
    );
    expect(readMetaContent('meta[property="og:description"]')).toBe(
      'Regarde Demo video pour River Quest à Phantasialand.',
    );
  ...
  it('uses explicit localized discovery terms for the French park content SEO', () => {
    service.applyParkItemsSeo(
      'Parc Démo',
      'fr',
      '/fr/park/park-1/parc-demo/items',
    );

    expect(documentRef.title).toBe(
      'Attractions, spectacles, restaurants et boutiques à Parc Démo — Amusement Parks',
    );
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Découvre les attractions, spectacles, restaurants, boutiques et lieux pratiques de Parc Démo.',
    );
  });
  ...
  it('uses explicit discovery terms for the English park content SEO', () => {
    service.applyParkItemsSeo(
      'Demo Park',
      'en',
      '/en/park/park-1/demo-park/items',
    );

    expect(documentRef.title).toBe(
      'Demo Park attractions, shows, restaurants and shops — Amusement Parks',
    );
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Browse attractions, shows, restaurants, shops and practical places at Demo Park.',
    );
  });

  it('applies indexable localized metadata to park opening hours pages without duplicating language copy', () => {
    service.applyParkOpeningHoursSeo(
      'Parc Demo',
      'fr',
      '/fr/park/park-1/parc-demo/opening-hours',
      12,
    );

    const frenchTitle: string = documentRef.title;
    const frenchDescription: string | null = readMetaContent(
      'meta[name="description"]',
    );

    expect(frenchTitle).toBe(
      'Dates et horaires de Parc Demo - Amusement Parks',
    );
    expect(frenchDescription).toContain('Parc Demo');
    expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
    expect(readCanonicalHref()).toBe(
      'http://localhost:4200/fr/park/park-1/parc-demo/opening-hours',
    );

    service.applyParkOpeningHoursSeo(
      'Demo Park',
      'en',
      '/en/park/park-1/demo-park/opening-hours?from=2026-07-01',
      12,
      null,
      '/en/park/park-1/demo-park/opening-hours',
    );

    expect(documentRef.title).toBe(
      'Dates and opening hours for Demo Park - Amusement Parks',
    );
    expect(readMetaContent('meta[name="description"]')).not.toBe(
      frenchDescription,
    );
  ...
  it('keeps history timeline SEO titles and descriptions unique between public languages', () => {
    const cases: Array<{
      ...
      {
        language: 'fr',
        title: 'Histoire de Mirapolis',
        url: '/fr/park/park-1/mirapolis/history',
      },
      {
        language: 'en',
        title: 'Mirapolis history',
        url: '/en/park/park-1/mirapolis/history',
      },
      {
        language: 'de',
        title: 'Geschichte von Mirapolis',
        url: '/de/park/park-1/mirapolis/history',
      },
      {
        language: 'nl',
        title: 'Geschiedenis van Mirapolis',
        url: '/nl/park/park-1/mirapolis/history',
      },
      {
        language: 'it',
        title: 'Storia di Mirapolis',
        url: '/it/park/park-1/mirapolis/history',
      },
      {
        language: 'es',
        title: 'Historia de Mirapolis',
        url: '/es/park/park-1/mirapolis/history',
      },
      {
        language: 'pl',
        title: 'Historia Mirapolis',
        url: '/pl/park/park-1/mirapolis/history',
      },
      {
        language: 'pt',
        title: 'História de Mirapolis',
        url: '/pt/park/park-1/mirapolis/history',
      },
    ];
    ...
    for (const entry of cases) {
      service.applyHistoryTimelineSeo(
        buildHistoryTimeline({ title: entry.title }),
        entry.language,
        entry.url,
      );
      ...
      if (entry.language === 'fr') {
        expect(documentRef.title).toBe(
          'Histoire de Mirapolis — Amusement Parks',
        );
      }
      expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
  ...
  it('adds park item context and image fallbacks to history timeline social metadata', () => {
    service.applyHistoryTimelineSeo(
      buildHistoryTimeline({
        ...
        park: buildPark({
          name: 'Dennlys Parc',
          currentLogoImageId: 'dennlys-logo-1',
        }),
        parkItem: buildParkItem({
          name: 'Le Nitro',
          mainImageId: 'nitro-photo-1',
        }),
        events: [
          {
    ...
    expect(documentRef.title).toBe(
      'Histoire de Le Nitro - Dennlys Parc — Amusement Parks',
    );
    expect(readMetaContent('meta[property="og:title"]')).toBe(
      'Histoire de Le Nitro - Dennlys Parc — Amusement Parks',
    );
    ...
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/nitro-photo-1/social-preview-v2',
    );
  ...
  it('adds pagination context to history timeline metadata and canonical URLs', () => {
    ...
    expect(readMetaContent('meta[name="description"]')).toContain(
      'Page 2 : repères 1990-1999.',
    );
    expect(readMetaContent('meta[property="og:description"]')).toContain(
      'Page 2 : repères 1990-1999.',
    );
  ...
  it('applies article Open Graph metadata to history articles and resets the type on regular pages', () => {
    ...
    expect(readMetaContent('meta[property="og:title"]')).toContain(
      'Article : Opening of Mirapolis',
    );
    expect(readMetaContent('meta[property="og:description"]')).toBe(
      'Article detaille sur l ouverture de Mirapolis.',
    );
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/history-photo-1/social-preview-v2',
    );
    expect(readMetaContent('meta[property="og:image:alt"]')).toBe(
      'Opening of Mirapolis',
    );
  ...
  it('keeps missing dynamic public routes noindex without canonical or social metadata', () => {
    ...
    service.applyRouteDefaults(
      '/fr/park/00000000-0000-0000-0000-000000000000/parc-manquant',
    );
  ...
  it('keeps static route titles and descriptions distinct across public languages', () => {
    ...
    for (const routeCase of routeCases) {
      ...
      expect(new Set(titles).size, `${routeCase.name} title`).toBe(
        languages.length,
      );
      expect(new Set(descriptions).size, `${routeCase.name} description`).toBe(
        languages.length,
      );
    }
  });
  ...
  it('keeps Open Graph titles and descriptions distinct across languages for shareable public pages', () => {
    ...
    const cases: Array<{
      ...
      },
      {
        ...
        apply: (language: string): void =>
          service.applyParkItemImagesSeo(
            buildParkItem({ name: 'Demo Item' }),
            buildPark({ name: 'Demo Park', countryCode: 'FR' }),
            language,
          ...
          service.applyParkReferenceSeo(
            buildParkReference({
              ...
              attractionsPagination: {
                totalItems: 4,
              } as ParkReferenceDetailViewModel['attractionsPagination'],
            }),
            language,
    ...
    for (const routeCase of cases) {
      ...
      expect(new Set(titles).size, `${routeCase.name} og:title`).toBe(
        languages.length,
      );
      expect(
        new Set(descriptions).size,
        `${routeCase.name} og:description`,
  ...
  it('localizes opening hours breadcrumb JSON-LD with contextual German labels', () => {
    service.applyParkOpeningHoursSeo(
      'Phantasialand',
      'de',
      '/de/park/park-1/phantasialand/opening-hours',
      12,
    );
  ...
  it('uses a clean canonical route for stale park zone slugs', () => {
    ...
    expect(documentRef.title).toBe(
      'Monaco themed area at Europa-Park — Amusement Parks',
    );
    expect(readCanonicalHref()).toBe(
      'http://localhost:4200/en/park/park-1/europa-park/zone/zone-1/monaco-themed-area',
    );
    expect(readMetaContent('meta[name="description"]')).toContain(
      '12 listed places',
    );
  ...
  it('applies specific metadata to public park reference detail pages', () => {
    ...
    expect(documentRef.title).toBe(
      'Ride Technic, constructeur — Amusement Parks',
    );
    expect(readCanonicalHref()).toBe(
      'http://localhost:4200/fr/park-manufacturer/manufacturer-1/ride-technic',
    );
    expect(readMetaContent('meta[name="description"]')).toContain(
      '4 attractions liées',
    );
  ...
  it('uses available attraction specs in fallback descriptions', () => {
    service.applyParkItemDetailSeo(
      buildParkItemDetail({
        ...
        zoneName: 'Rookburgh',
        spotlightRows: [{ labelKey: 'speed', value: '78 km/h' }],
        accessConditions: [
          {
            metrics: [
              {
    ...
    const description: string | null = readMetaContent(
      'meta[name="description"]',
    );

    expect(readCanonicalHref()).toBe(
      'http://localhost:4200/fr/park/park-1/phantasialand/item/item-1/f-l-y',
    );
  ...
  it('applies indexable localized metadata to the public rankings page', () => {
    ...
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Découvre les parcs, attractions, restaurants, hôtels et services les plus régulièrement appréciés des visiteurs.',
    );
    expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
  });
  ...
  it('applies indexable localized metadata to the public technical pages list', () => {
    ...
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Explore les lifts, retenues, trains, matériaux et autres systèmes techniques des attractions.',
    );
    expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
    expect(readMetaContent('meta[property="og:locale"]')).toBe('fr_FR');
  });
  ...
  it('applies localized metadata to a technical detail page', () => {
    service.applyTechnicalPageSeo(
      buildTechnicalPage(),
      'fr',
      '/fr/technical/lap-bar',
    );

    expect(documentRef.title).toBe(
      'Dossier technique : Lap bar - Amusement Parks',
    );
    expect(readMetaContent('meta[name="description"]')).toBe(
      'Explication technique de la lap bar.',
    );
    expect(readMetaContent('meta[property="og:image"]')).toBe(
      'https://localhost:44391/images/binary/technical-photo-1/social-preview-v2',
    );
    expect(readMetaContent('meta[property="og:url"]')).toBe(
  ...
  it('applies indexable localized metadata to the public manufacturers page', () => {
    ...
    expect(readMetaContent('meta[name="description"]')).toBe(
      "Parcours les constructeurs d'attractions et de coasters avec leur fiche publique, leur histoire et leurs liens utiles.",
    );
    expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
  });
  ...
  it('applies indexable interactive map metadata to public park map pages', () => {
    ...
    expect(readMetaContent('meta[name="robots"]')).toBe('index,follow');
    expect(readCanonicalHref()).toBe(
      'http://localhost:4200/fr/park/park-1/parc-demo/map',
    );
  });
  ...
  function readCanonicalHref(): string | null {
    return (
      documentRef.head.querySelector<HTMLLinkElement>('link[rel="canonical"]')
        ?.href ?? null
    );
  }
  ...
  function readOpenGraphLocaleAlternates(): string[] {
    return Array.from(
      documentRef.head.querySelectorAll<HTMLMetaElement>(
  ...
  function readBreadcrumbNames(): string[] {
    ...
    return elements.map((element: Record<string, unknown>): string =>
      String(element['name'] ?? ''),
    );
  }
  ...
  function readBreadcrumbElements(): Array<Record<string, unknown>> {
    const breadcrumb = readJsonLdScripts().find(
  ...
  function readJsonLdScripts(): Array<Record<string, unknown>> {
    return Array.from(
      documentRef.head.querySelectorAll<HTMLScriptElement>(
        'script[data-managed-by="amusementpark-seo"]',
      ),
    ).map(
      (script: HTMLScriptElement): Record<string, unknown> =>
...
function buildParkDetail(
  overrides: Partial<ParkDetailViewModel> = {},
): ParkDetailViewModel {
  return {
    name: 'Demo Park',
    status: 'Operating',
...
function buildParkItemDetail(
  overrides: Partial<ParkItemDetailViewModel> = {},
...
function buildHistoryTimeline(
  overrides: Partial<HistoryTimelinePageViewModel> = {},
): HistoryTimelinePageViewModel {
  return {
    entityType: 'Park',
    title: 'Mirapolis history',
...
function buildHistoryArticle(
  overrides: Partial<HistoryArticlePageViewModel> = {},
): HistoryArticlePageViewModel {
  return {
    title: 'Opening of Mirapolis',
    subtitle: '',
...
function buildParkReference(
  overrides: Partial<ParkReferenceDetailViewModel> = {},
...
function buildTechnicalPage(
  overrides: Partial<TechnicalPage> = {},
): TechnicalPage {
  return {
