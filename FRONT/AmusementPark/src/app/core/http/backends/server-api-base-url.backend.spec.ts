...
describe('ServerApiBaseUrlBackend', () => {
  function handleUrl(url: string): FetchBackendFake {
    ...
    const backend: ServerApiBaseUrlBackend = new ServerApiBaseUrlBackend(
      fetchBackend as unknown as FetchBackend,
    );
  ...
  it('rewrites browser API urls to the SSR API base url and flags the internal call', () => {
    const result: FetchBackendFake = handleUrl(
      `${environment.apiBaseUrl}api/parks?page=1`,
    );

    expect(result.capturedUrl).toBe(
      `${environment.ssrApiBaseUrl}api/parks?page=1`,
    );
    expect(result.capturedInternalSsrHeader).toBe('1');
  });

  it('leaves already external or relative urls untouched', () => {
    expect(handleUrl('https://cdn.example.test/image.png').capturedUrl).toBe(
  ...
  it('retries transient safe internal API failures during SSR', async () => {
    const retryAfterHeaders: HttpHeaders = new HttpHeaders({
      'Retry-After': '0',
    });
    ...
    const event: HttpEvent<unknown> = await firstValueFrom(
      backend.handle(
        new HttpRequest('GET', `${environment.apiBaseUrl}api/videos/video-1`),
      ),
    );
  ...
  it('does not retry permanent not found responses during SSR', async () => {
    const notFoundError: HttpErrorResponse = new HttpErrorResponse({
      status: 404,
    });
    const fetchBackend: FetchBackendFake = new FetchBackendFake(() =>
    ...
    backend
      .handle(
        new HttpRequest('GET', `${environment.apiBaseUrl}api/videos/missing`),
      )
      .subscribe({
        next: () => {
          throw new Error('Expected the 404 response to stay an error.');
        },
  ...
  it('does not retry unsafe internal API methods during SSR', async () => {
    const serviceUnavailableError: HttpErrorResponse = new HttpErrorResponse({
      status: 503,
    });
    const fetchBackend: FetchBackendFake = new FetchBackendFake(() =>
      throwError(() => serviceUnavailableError),
    );
    const backend: ServerApiBaseUrlBackend = new ServerApiBaseUrlBackend(
      fetchBackend as unknown as FetchBackend,
    );
    ...
    backend
      .handle(
        new HttpRequest(
          'POST',
          `${environment.apiBaseUrl}api/social-share/events`,
          {},
        ),
      )
      .subscribe({
        next: () => {
          throw new Error('Expected the unsafe request to stay an error.');
        },
        error: (error: HttpErrorResponse) => {
          expect(fetchBackend.subscriptionCount).toBe(1);
