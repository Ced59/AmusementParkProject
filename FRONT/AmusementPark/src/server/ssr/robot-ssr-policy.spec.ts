...
describe('robot SSR policy', () => {
  it('allows cold SSR for search, user-triggered, audit and social preview robots', () => {
    ...
    allowedFamilies.forEach((family: RobotFamily | null) => {
      expect(
        shouldAllowRobotCacheMissSsrRender(family),
        `${family ?? 'human'} should render`,
      ).toBe(true);
    });
  });
  ...
  it('distinguishes official AI search, user-triggered and training agents', () => {
    ...
    cases.forEach(
      ([userAgent, expectedFamily]: readonly [string, RobotFamily]) => {
        expect(detectRobotFamilyFromUserAgent(userAgent), userAgent).toBe(
          expectedFamily,
        );
      },
    );
  ...
  it('detects established crawler and preview families before falling back to Other bot', () => {
    expect(detectRobotFamilyFromUserAgent('Mozilla/5.0 Applebot/0.1')).toBe(
      'Applebot',
    );
    expect(
      detectRobotFamilyFromUserAgent(
    ...
    expect(detectRobotFamilyFromUserAgent('Mozilla/5.0 PetalBot')).toBe(
      'PetalBot',
    );
    expect(detectRobotFamilyFromUserAgent('Mozilla/5.0 AhrefsBot')).toBe(
      'AhrefsBot',
    );
    expect(
      detectRobotFamilyFromUserAgent(
    ...
    ).toBe('AhrefsSiteAudit');
    expect(detectRobotFamilyFromUserAgent('facebookexternalhit/1.1')).toBe(
      'Facebook external hit',
    );
    expect(detectRobotFamilyFromUserAgent('WhatsApp/2.24')).toBe('WhatsApp');
    expect(detectRobotFamilyFromUserAgent('Mozilla/5.0 SomeCrawler')).toBe(
      'Other bot',
    );
  });
  ...
  it('tracks Mariner as a robot without stripping scripts from its interactive browser session', () => {
    expect(detectRobotFamilyFromUserAgent('GoogleAgent-Mariner/1.0')).toBe(
      'GoogleAgent-Mariner',
    );
    expect(shouldAllowRobotCacheMissSsrRender('GoogleAgent-Mariner')).toBe(
      true,
    );
  ...
  it('does not treat internal targeted refreshes as robot traffic', () => {
    const family: RobotFamily | null = detectRobotFamilyFromUserAgent(
      'AmusementPark-SSR-TargetedRefresh/1.0',
    );

    expect(family).toBeNull();
    expect(shouldAllowRobotCacheMissSsrRender(family)).toBe(true);
  });
});
