...
Le frontend place ces actions dans l'éditeur existant. Les formulaires et mutations de contenu ne sont disponibles qu'en brouillon ; une visite terminée ou archivée reste lisible et doit être rouverte volontairement. L'ajout d'occurrences attend aussi l'enregistrement d'une date, d'une précision, d'un fuseau ou d'une convention de journée modifiés, afin de ne jamais créer un enfant avec l'identité temporelle précédente. La grille passe à une colonne sous 620 px, chaque conteneur possède `min-width: 0`, les textes longs peuvent se couper et aucun champ ou bouton ne peut élargir le viewport.
...
Le bail porte en plus une génération persistante et monotone, `contentMutationFenceToken`. Une acquisition normale réutilise la génération déjà prête lorsque le précédent détenteur a explicitement confirmé que toutes ses écritures ont reçu une réponse définitive. Elle ne modifie alors que le document de visite qui porte le bail : aucun historique d'occurrences ou d'opérations n'est réécrit. La première utilisation, un bail expiré, une promotion incomplète ou une portée libérée sans confirmation déclenchent au contraire une récupération : la génération est incrémentée et `contentMutationFenceReady` passe temporairement à `false`. Chaque écriture d'occurrence et chaque transition d'opération exige la génération exacte obtenue avec le bail. `contentMutationFenceStableToken` mémorise la dernière génération entièrement publiée. Le détenteur de récupération ne promeut que les descendants compris entre cette génération stable et la nouvelle génération : une écriture très ancienne arrivée après sa barrière reste donc exclue. Il promeut les occurrences puis les opérations avant de publier la nouvelle génération avec un `updateOne` exigeant le token de bail exact, la génération exacte et une échéance encore valide. Il n'utilise la nouvelle génération que si cet acquittement réussit, puis avance la génération stable.

Dès que la génération est déclarée prête, les lectures n'exposent que ses descendants et l'audit ignore toute génération obsolète. Pendant une promotion incomplète, les lectures et la reprise d'audit restent bornées à l'intervalle sûr entre la dernière génération stable et la génération en cours : une preuve déjà durable ne reste donc pas bloquée si le processus s'arrête avant l'acquittement final. La prochaine acquisition crée une autre génération et recommence la barrière avant toute mutation. Une allocation de création clôturée conserve un marqueur provisoire indexé jusqu'au passage du réconciliateur : celui-ci retire le marqueur si l'opération exacte est `completed` sous le même fence, attend si elle est encore récupérable, et supprime seulement ce document exact dans les autres états. Comme Mongo promeut d'abord les occurrences puis leur opération, un document momentanément en avance n'est jamais supprimé lorsque ses deux générations appartiennent à l'intervalle stable-courant d'une promotion incomplète : si l'opération exacte est déjà `completed`, le marqueur est validé immédiatement ; si elle est encore `pending`, le réconciliateur attend sa reprise. Hors de cet intervalle, il relit l'opération avant toute décision destructive. Une création tardive n'est donc jamais publiée comme un faux succès ; si elle arrive même après le rejet et la première purge, son propre marqueur la rend visible au prochain lot de nettoyage borné. Cette barrière persistante protège même une écriture Mongo déjà envoyée qu'une simple annulation en mémoire ne pourrait plus arrêter.

Le détenteur renouvelle toutes les minutes son échéance de cinq minutes avec un filtre exigeant son token exact et une échéance non expirée. Une sortie normale marque la portée comme définitivement réglée avant sa libération. Une exception, une annulation ou une perte de propriété laisse au contraire la génération dans un état exigeant une récupération ; la prochaine acquisition crée alors la barrière suivante avant toute nouvelle mutation. Les filtres de génération refusent toute écriture déjà envoyée qui arriverait après cette reprise. Avant toute promotion ciblée d'une opération idempotente, le repository exige l'utilisateur et la visite d'origine : une même clé réutilisée sur une autre visite produit un conflit sans déplacer l'opération ni ses occurrences vers le fence numérique de cette seconde visite. Un bail réellement abandonné reste récupérable après cinq minutes.
...
sequenceDiagram
    ...
    opt mutation d'une occurrence
        ...
        alt génération stable après une portée confirmée
            L->>S: pose seulement le nouveau token de bail
            L-->>H: bail + génération stable réutilisée
        else première utilisation ou récupération nécessaire
    ...
    opt bail acquis
        H->>L: confirme la fin définitive de la portée
