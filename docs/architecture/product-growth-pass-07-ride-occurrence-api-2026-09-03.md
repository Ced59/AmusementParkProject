...
Les deux créations et le réordonnancement exigent `Idempotency-Key`. Une réponse rejouée conserve le même contrat et ajoute `Idempotency-Replayed: true`. Une renormalisation rare ajoute `Ride-Order-Normalized: true` sur le déplacement comme sur l'ajout qui l'a déclenchée : le client sait ainsi invalider les versions antérieures qu'il aurait en cache. La politique CORS autorise explicitement la clé requise et expose les deux signaux de rejeu/normalisation aux clients navigateur approuvés. Avant toute lecture ou mutation d'ordre, Application réserve le hash canonique partagé de la clé avec l'état `reserved`. Le document fige le contexte minimal de visite et les décisions de cohérence historique déjà validées ; il ne verrouille pas encore la visite, mais empêche atomiquement une autre création ou un réordonnancement de réutiliser la même clé. Un retry strictement identique retrouve cette préparation avant de relire la visite ou les attractions mutables et peut donc reprendre même si elles ont ensuite été modifiées ou masquées. Une fois les positions disponibles, le même document passe atomiquement de `creation-key-reservation/reserved` à `creation/pending` et acquiert alors le verrou de visite ; un plan d'append obsolète est reconverti en réservation au lieu de supprimer cette preuve. Le signal de normalisation est conservé dans cette réservation de création finale et reste donc présent sur son replay. L'opération interne de renormalisation conserve aussi, avant sa première écriture, l'empreinte de la création qui l'a déclenchée. La lecture de l'état d'append retrouve cette preuve durable ; l'infrastructure la vérifie de nouveau juste après avoir acquis la réservation exclusive de création et la persiste avant toute occurrence. Même si le processus s'arrête à cet endroit, un retry consolide la même preuve sur la réservation retrouvée. L'empreinte de création porte sur toute la requête normalisée, y compris la confirmation explicite d'un conflit historique, et est recherchée avant la visite ou la cible mutable : un retry strictement identique reste donc rejouable même si l'attraction a ensuite été masquée, déplacée ou supprimée, tandis qu'une même clé associée à un consentement ou un contenu différent produit un conflit d'idempotence. Le `Location` d'une création unitaire pointe vers la lecture GET propriétaire de cette occurrence ; une autre visite ou un autre propriétaire reçoit `404`.
...
Si la réponse réseau est perdue après une écriture, le même retry reconnaît les lignes déjà appliquées grâce à `lastReorderOperationKeyHash`, y compris après une correction ultérieure qui a augmenté leur version, termine les lignes restantes puis rejoue le snapshot réservé. Dès que l'occurrence déplacée est absente des allocations — plan entièrement sans changement ou renormalisation qui ne modifie que ses voisines — elle pose tout de même ce marqueur par un CAS sur la version attendue : elle n'incrémente pas la version métier, mais possède un point de linéarisation et ne peut pas répondre avec un snapshot déjà périmé. Si une version réellement concurrente bloque une ligne, le worker recharge d'abord l'état durable : une opération déjà terminée est rejouée sans compensation. Sinon, un CAS réserve la compensation tout en laissant l'opération `pending`, donc la visite verrouillée ; la complétion concurrente devient impossible et les mutations déjà appliquées sont restaurées en ordre inverse. L'opération n'est marquée `conflict` qu'après ce rollback complet ; une interruption laisse la compensation `pending` et récupérable par la même clé. Une réutilisation de la clé avec un autre déplacement retourne `ride-occurrence.idempotency-key-conflict`, distinct d'un conflit de version récupérable par rechargement.
...
user-ride-occurrence-operations:
  operationKind: creation-key-reservation | creation | reorder | delete
  creationPreparation?: {
    parkId: string,
    visitDate: date partielle structurée,
    timeZoneId?: string,
    serviceDayConvention: VisitStartLocalDate,
    items: [{ index: int32, historicalConsistency: enum }]
  }
  items: [{
    index: int32,
    occurrenceId: string,
  ...
  visitId?: string
  operationState?: reserved | pending | completed | conflict
  ...
  reorderCompensationStarted?: boolean
  reorderResultSnapshot?: occurrence déplacée renvoyée au client
```

La clé client brute n'est jamais stockée. La réservation préalable et l'opération finale utilisent exactement le même SHA-256 canonique. L'index unique `(userId, operationKeyHash)` arbitre donc sans fenêtre les réservations, créations, déplacements et identifiants internes de suppression ; l'index unique partiel `(userId, visitId)` couvre seulement les opérations `pending`, quel que soit leur type, pour sérialiser la timeline d'une visite sans qu'une préparation `reserved` bloque les autres visites. Un index partiel `(userId, relatedCreationOperationKeyHash, visitId, operationState)` retrouve en coût borné la renormalisation terminée liée à un ajout interrompu. Chaque allocation de réordonnancement ne conserve que les trois champs nécessaires à son application et à sa compensation ; la note privée et les autres données complètes ne sont présentes qu'une fois dans `reorderResultSnapshot`. Même à la borne de 2 000 lignes avec une note UTF-8 maximale, le document de réservation testé reste inférieur à 1 Mo, très en dessous de la limite BSON de 16 Mio.
...
- Application : expansion du nombre, propriété, validation cible/parc/catégorie, confirmation historique, versions, réservation de clé avant toute opération d'ordre, replay avant les dépendances mutables et normalisation avant ajout à la borne `long` ;
