...
- `codex-park-data-editor-api-workflow.md` : complément strictement réservé à Codex lorsqu’il exécute lui-même les étapes par API avec le rôle technique `PARK_DATA_EDITOR`. Il ajoute les garde-fous Preview/Apply et l’upload local des photos et cartes officielles sans modifier le workflow ChatGPT.
...
- L'absence de formulation mécanique vaut pour **tout le corpus public et chacune des huit langues** : parc, zones, parkItems de toute catégorie, histoire, articles, médias, titres, sous-titres, résumés, descriptions, textes alternatifs et légendes. Une seule tournure qui ressemble à un gabarit, à une taxonomie injectée ou à un champ rempli automatiquement bloque l'audit final ; l'absence de doublon exact ou d'alerte automatisée ne suffit jamais à la valider.
...
- Écrire les apostrophes, guillemets, espaces, accents et autres caractères destinés au visiteur directement en Unicode. Ne jamais enregistrer `&rsquo;`, `&#8217;`, `&eacute;`, `&nbsp;` ou une autre entité HTML de présentation dans un champ public. Dans le HTML riche, seuls `&amp;`, `&lt;` et `&gt;` restent admis lorsqu’ils protègent réellement la syntaxe ; tous les autres caractères restent littéraux.
...
- Rechercher et vérifier le logo officiel actuel comme un élément distinct de la photo principale. Il doit devenir le logo courant, rester sans watermark ajouté et être contrôlé dans l’export final. Le logo est obligatoire par défaut et une photo de parc ne peut jamais le remplacer. Une exception n'est envisageable que pour un très petit parc après recherche infructueuse documentée dans les canaux officiels, les archives de l'exploitant et les sources spécialisées, puis acceptation explicite de la lacune finale.
- Rechercher les cartes officielles publiées par le parc ou son exploitant, commencer par l’édition actuelle puis remonter les millésimes réellement disponibles. Conserver chaque fichier accepté dans le stockage objet avec son année et sa page source ; ne jamais inventer une année, contourner une restriction d’accès ni substituer une carte communautaire.
- Viser au moins cinq photographies générales distinctes du parc, hors logo, montrant réellement le parc dans son ensemble ou plusieurs de ses espaces identifiables. Un gros plan d’attraction, une image générique ou un visuel promotionnel qui ne situe pas le parc ne compte pas dans ce minimum.
- Viser au moins trois photographies distinctes pour chaque parkItem important, actuel ou historique, afin d’en montrer plusieurs vues ou contextes utiles sans dupliquer artificiellement un même cadrage. L’importance doit être justifiée par la place de l’entité dans l’offre, l’identité ou l’histoire du parc.
- Ces minima constituent la norme lorsque des fichiers fiables, créditables et importables existent. Une source secondaire est admissible si l’image montre sans ambiguïté la bonne entité, a été inspectée visuellement et ne porte aucun watermark. Pour un petit parc ou un sujet peu documenté, une exception est possible seulement après une recherche réelle : consigner le nombre atteint, les familles de sources vérifiées et la raison factuelle de l’écart, sans utiliser d’image générique ou trompeuse.
- Rechercher en plus une image fidèle pour chaque attraction actuelle, annoncée, en construction ou définitivement fermée, ainsi que pour chaque jalon et article historique, quand une image acceptable existe.
- Pour une image non officielle, conserver la page source, l'auteur et la licence ou l'autorisation de réutilisation, puis contrôler visuellement le parc, l'attraction et le contexte. Un résultat de recherche, une miniature sociale, un nom de fichier ou une légende tierce ne prouve ni l'identité ni le droit d'usage.
