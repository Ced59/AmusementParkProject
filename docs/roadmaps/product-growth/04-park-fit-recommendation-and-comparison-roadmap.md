...
comparateur. Coordonnées, calendrier, attractions visibles actuellement ouvertes, conditions d'accès,
...
Le lecteur compatible a d'abord donné à tout état absent la sémantique
`NotActivated`, sans écrire une valeur inconnue des anciennes instances. Une seconde
livraison, déclenchée après le déploiement vérifié de ce lecteur, ouvre maintenant
l'activation et le retrait et matérialise physiquement chaque état absent en
`NotActivated` par lots idempotents. Les états actifs ou suspendus et leur historique
ne sont jamais écrasés ; aucun adaptateur conservant l'ancien « absent = actif » ne
subsiste. La recherche utilise une projection MongoDB dédiée :
...
L'administration distingue les trois états, explique les blocages de qualité et
propose les quatre actions métier dans les huit langues : activer, suspendre,
rétablir et retirer. Elle reste contenue sur mobile. L'architecture, les schémas
MongoDB, les diagrammes de
