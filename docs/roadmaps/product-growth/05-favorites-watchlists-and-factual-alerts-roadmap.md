...
| `WATCH-12` | Pilote et métriques — livré en `5.3.56` | Gate mesurable |
...
`WATCH-12` rend le pilote observable sans profilage individuel. Un cockpit administratif responsive agrège abonnements actifs par type, cycle des faits, notifications, doublons, latence de livraison, digests, désabonnements observés, ouvertures du centre, consultations de sources, signalements, livraisons e-mail, outbox et files techniques. Les interactions produit ne conservent que des compteurs quotidiens partagés et toutes les séries d'une requête utilisent les mêmes journées civiles UTC. Les doublons comptés sont les livraisons effectivement empêchées par la clé logique, et non des documents impossibles à retrouver après l'action de l'index unique. Le signalement d'une même notification privée est idempotent ; son horodatage persistant constitue directement la source de vérité de la métrique et suit la rétention et la suppression déjà appliquées à cette notification. La décision d’étendre les types appartient au domaine et reste volontairement fermée quand le volume est insuffisant, qu’un seuil de qualité est dépassé ou que le fournisseur SMTP ne fournit pas encore les rebonds et plaintes : une absence de donnée est affichée comme telle, jamais transformée en succès. Ce constat n’empêche pas de poursuivre les autres roadmaps techniques et métier.
