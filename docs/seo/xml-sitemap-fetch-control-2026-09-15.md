...
- Si le snapshot principal tient dans les bornes mais que la copie supplémentaire les dépasserait, il est publié sans témoin, avec `document-limit` ou `byte-limit`. Ces états invalident l’expérience ; ils ne bloquent pas la publication principale. Un dépassement par les documents principaux conserve son erreur habituelle.
