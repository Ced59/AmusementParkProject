...
public sealed class CorrectFactualChangeEventCommandHandler
    : ICommandHandler<CorrectFactualChangeEventCommand, ApplicationResult>
{
    ...
    private readonly ILogger<CorrectFactualChangeEventCommandHandler> logger;

    public CorrectFactualChangeEventCommandHandler(
        FactualChangeEventAdministrationService service,
        IFactualNotificationDistributionScheduler scheduler,
        ILogger<CorrectFactualChangeEventCommandHandler> logger)
    {
    ...
    public async Task<ApplicationResult> HandleAsync(
        CorrectFactualChangeEventCommand command,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
        ApplicationResult result = await this.service.CorrectAsync(
            command.EventId,
            command.ExpectedVersion,
            command.SupersedingEventId,
            cancellationToken);
        if (result.IsSuccess)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception)
        {
            this.logger.LogError(
                exception,
                "The factual event {FactualEventId} was corrected, but redistribution scheduling failed; reconciliation will retry.",
                eventId);
        }
    }
}
