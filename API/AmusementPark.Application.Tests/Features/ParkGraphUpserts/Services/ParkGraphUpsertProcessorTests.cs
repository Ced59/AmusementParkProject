...
{
    [Fact]
    public void Constructors_ShouldExposeSingleGreediestDependencyInjectionConstructor()
    {
        System.Reflection.ConstructorInfo[] constructors = typeof(ParkGraphUpsertProcessor).GetConstructors();
        int maximumParameterCount = constructors.Max(static constructor => constructor.GetParameters().Length);
        System.Reflection.ConstructorInfo dependencyInjectionConstructor = Assert.Single(
            constructors,
            constructor => constructor.GetParameters().Length == maximumParameterCount);

        Assert.Contains(
            dependencyInjectionConstructor.GetParameters(),
            static parameter => parameter.ParameterType == typeof(IParkPricingRepository));
        Assert.Contains(
            dependencyInjectionConstructor.GetParameters(),
        ...
        bool expectedPricingMigration)
    {
        ...
        parkRepository
            ...
            {
                targetParkReadCount++;
                return targetParkReadCount >= 3 && persistedTargetPark is not null
                    ? persistedTargetPark
                    : targetPark;
            });
        parkRepository
        ...
        parkRepository
            .Setup(value => value.UpdateAsync("park-target", It.IsAny<Park>(), It.IsAny<CancellationToken>()))
            .Callback<string, Park, CancellationToken>((_, value, _) =>
            {
                persistedTargetPark = value;
                updatedParkNames.Add(value.Name);
            })
            .ReturnsAsync((string _, Park value, CancellationToken _) => value);
        parkRepository
            .Setup(value => value.DeleteAsync("park-source", It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        ...
        parkItemRepository
            .Setup(value => value.GetByParkIdAsync("park-source", true, It.IsAny<CancellationToken>()))
        ...
        searchProjectionWriter
            ...
            .Returns(Task.CompletedTask);
        searchProjectionWriter
            .Setup(value => value.UpsertAsync(SearchProjectionResourceTypes.Parks, "park-target", It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        Assert.Equal("park-target", result.Value!.TargetParkId);
        Assert.Equal("Source Park", result.Value.TargetParkName);
    ...
    [Fact]
    public async Task ApplyAsync_WhenSupprContainsStoredImage_ShouldDeleteBinaryBeforeMetadata()
    {
        Park park = new Park
        {
            Id = "park-1",
        ...
        MockSequence deletionSequence = new MockSequence();
        Mock<IImageBinaryStorage> imageBinaryStorage = new Mock<IImageBinaryStorage>(MockBehavior.Strict);
        imageBinaryStorage
            .InSequence(deletionSequence)
            .Setup(value => value.DeleteAsync("parks/park-1/duplicate", It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        ...
        imageRepository
            .InSequence(deletionSequence)
            .Setup(value => value.DeleteAsync("image-1", It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        ...
        ParkGraphUpsertProcessor processor = new ParkGraphUpsertProcessor(
            ...
            publicSeoUpdateNotifier.Object,
            MeasurementConversionService.Instance,
            imageBinaryStorage: imageBinaryStorage.Object);
    ...
    [Fact]
    public async Task ApplyAsync_WhenStoredImageBinaryCannotBeDeleted_ShouldKeepMetadataAndReportError()
    {
        Park park = new Park
        {
            Id = "park-1",
        ...
        Assert.Equal(0, result.Value.Counts.Deleted);
    ...
    [Fact]
    public async Task ApplyAsync_WhenSupprContainsParkItemThenOwnedImage_ShouldDeleteBoth()
    {
        Park park = new Park
        {
            Id = "park-1",
        ...
        parkItemRepository
            .Setup(value => value.DeleteAsync("item-1", It.IsAny<CancellationToken>()))
            .Callback<string, CancellationToken>((_, _) => itemDeleted = true)
            .ReturnsAsync(true);
        ...
        imageRepository
            .Setup(value => value.DeleteAsync("image-1", It.IsAny<CancellationToken>()))
            .Callback<string, CancellationToken>((_, _) => Assert.True(itemDeleted))
            .ReturnsAsync(true);
        ...
        Assert.True(result.IsSuccess);
        Assert.Contains(result.Value!.Changes, change => change.EntityType == "ParkItem" && change.EntityId == "item-1" && change.ChangeType == "Deleted");
        Assert.Contains(result.Value.Changes, change => change.EntityType == "Image" && change.EntityId == "image-1" && change.ChangeType == "Deleted");
        Assert.Equal(2, result.Value.Counts.Deleted);
        parkRepository.VerifyAll();
        parkItemRepository.VerifyAll();
        imageRepository.VerifyAll();
        searchProjectionWriter.VerifyAll();
    ...
    [Fact]
    public async Task ApplyAsync_WhenOneDeletionTargetCannotBeResolved_ShouldNotDeleteAnyResolvedTarget()
    {
        Park park = new Park
        {
            Id = "park-1",
        ...
        Mock<IImageRepository> imageRepository = new Mock<IImageRepository>(MockBehavior.Strict);
        imageRepository
            .Setup(value => value.GetByIdAsync("missing-image", It.IsAny<CancellationToken>()))
            .ReturnsAsync((Image?)null);
    ...
    [Fact]
    public async Task ApplyAsync_WhenImperialOnlyPatchUpdatesExistingAttraction_ShouldUpdateMetricTruth()
    {
        Park park = new Park
        {
            Id = "park-1",
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(updatedItem?.AttractionDetails);
        Assert.Equal(60.96d, updatedItem.AttractionDetails.HeightInMeters);
        Assert.Equal(200d, updatedItem.AttractionDetails.HeightInFeet);
        Assert.Contains(result.Value!.Changes.SelectMany(change => change.Fields), field => field.Field == "attractionDetails.heightInMeters" && field.NewValue == "60.96");
        parkRepository.VerifyAll();
        parkItemRepository.VerifyAll();
        searchProjectionWriter.VerifyAll();
