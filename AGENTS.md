...
- When an automatic backlog park announcement is `Failed`, retry only the same server-returned publication with `RetryFacebookPublication` and the exact park ID. Never call `PublishFacebook` again or create a manual duplicate to bypass the failure. Resolve the draft again after the retry, retain the backlog row unless the announcement is `Published`, and report any repeated failure.
