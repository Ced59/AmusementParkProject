...
- The bounded exception is a park that is already visible and selected from `docs/roadmaps/published-parks-completeness-backlog-2026-08-13.md`: follow `docs/codex-guidelines/published-park-backlog-workflow.md`. After a fresh final audit without blockers and an individually recalculated score strictly above 95, publish the audited data, resolve the Facebook draft, publish the default park announcement only when no published announcement exists, and remove the backlog row only after both publication outcomes are successful.
