...
- The short completion request authorizes autonomous execution of every applicable step, including research, bounded Preview/Apply lots, the initial export, private image imports, the refreshed full export immediately before step 8 and the final audit. It does not authorize publication of the park, its new content or its media, deletion, hiding previously public content, or cleanup of an unknown legacy entity.
- In the autonomous Codex API workflow, never run a full park export after each Apply, image import, lot or transition between steps 1 through 7. Maintain a consolidated local working state from the initial export and the successful Preview, Apply and image-import responses. Outside the initial export, the only scheduled mandatory full export is the fresh export immediately before the step 8 audit; use a section-limited export earlier only to resolve a specific proven inconsistency or a missing mutation response.
