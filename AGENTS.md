...
- The short completion request authorizes autonomous execution of every applicable step, including research, bounded Preview/Apply lots, private image imports, the single full export immediately before step 8 and the final audit. It does not authorize publication of the park, its new content or its media, deletion, hiding previously public content, or cleanup of an unknown legacy entity.
- In the autonomous Codex API workflow, no full park export is mandatory during steps 0 through 7, including at intake. Maintain a consolidated local working state from park search results, any narrowly scoped exports that are genuinely needed, and the successful Preview, Apply and image-import responses. The only scheduled mandatory full export is the fresh export immediately before the step 8 audit; use a section-limited export earlier only to resolve a specific identity, ambiguity, missing ID, dependency or lost mutation response.
